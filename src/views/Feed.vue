...
<template>
  <br><br>
  <h1 class="title-heading text-dark">News Feed</h1>
  <br><br>
  <div v-if="isLoading">
    <Spinner />
  </div>
    <div v-else class="card">
    <div class="" v-for="post in userPosts" :key="post.id">
      <div class="card-header">
        <span class="float-end "><b><i>{{post.category }}</i></b></span>
          <h3 class="text-white">{{post.authors}}</h3>
             </div>                  
     
      <div class="card-body" style="height: 300px;">
         <h5 class="text-center text-dark" style="font-family: serif">{{ post.title }}</h5>
       
         <!-- <div class="card-avatar"></div>  --><br>
          <p class=" text-center text-dark bg-light" style="font-family: sans-serif; border-radius:10%;"><i class="fa fa-quote-left" aria-hidden="true"></i>&nbsp;&nbsp;{{ post.description }}&nbsp;&nbsp;<i class="fa fa-quote-right" aria-hidden="true"></i></p>
        <div class="card-subtitle">{{ post.category }}</div>
    
        <!-- <div class="card-footer">
          <span>Posted at: {{ post.createdAt -format('F d, Y | g:i A') }}</span>
                      </div> -->
